...
class GroceryDatabase {
  ...
  async addItem(listId: number, text: string): Promise<void> {
    ...
    try {
      await this.db.runAsync(
        "INSERT INTO items (listId, text) VALUES (?, ?);",
        [listId, text],
      );
    } catch (error) {
  ...
  async updateItem(id: number, checked: boolean): Promise<void> {
    ...
    try {
      await this.db.runAsync("UPDATE items SET checked = ? WHERE id = ?;", [
        checked ? 1 : 0,
        id,
      ]);
    } catch (error) {
   ...
   */
  async fetchItems(listId: number): Promise<Array<Item>> {
    if (!this.db) {
      throw new Error("Database not initialized");
    }
    try {
      return await this.db.getAllAsync<{
        ...
        text: string;
        checked: number;
      }>("SELECT * FROM items WHERE listId = ?;", [listId]);
    } catch (error) {
   ...
   *
   * @returns {Promise<Array<List>>} A promise that resolves to an array of lists.
